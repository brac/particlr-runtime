// Pixi v7 adapter tests. A port of test/pixi/renderer.test.ts adapted to v7
// facts: children are Sprites (pc.children, not pc.particleChildren); scale is a
// Point (part.scale.x/.y, not part.scaleX/.scaleY); ParticleContainer has no
// `.texture` property (the view tracks the layer texture); `_properties` is a
// boolean[] ([vertices, position, rotation, uvs, tint]).
//
// This file runs in the vitest `pixi7` project ONLY, whose `pixi.js` alias
...
// White-box view of a layer's render state. Mirrors the `_properties` cast used
// in the guards below — a unit test may read the renderer's private slices to
// prove the wiring without exposing them as public API. `texture` stands in for
// v8's `pc.texture` (v7's ParticleContainer has none).
interface ViewProbe {
  frames: Texture[] | null;
...
// v7 `_properties` is a boolean[] in fixed order: [vertices, position, rotation,
// uvs, tint]. (v8 used a keyed Record.)
const propsOf = (pc: ParticleContainer): boolean[] =>
  (pc as ParticleContainer & { _properties: boolean[] })._properties;
const uvsDynamic = (pc: ParticleContainer): boolean => propsOf(pc)[3]!;

// --- runtime-resolution trap (reviewer-added, PIXI7_PLAN M1) ----------------
describe("pixi7 adapter resolves pixi.js v7 at runtime, not v8", () => {
  it("imports pixi.js@7.4.3 inside the pixi7 vitest project", () => {
    // The vite alias redirects `import "pixi.js"` to node_modules/pixi7 for this
    // project; VERSION is the v7 monolith's own constant. If the alias were
    // missing we'd get v8 (8.19.0) here and silently test the wrong major.
  ...
  it("pins the alias package (node_modules/pixi7) at 7.4.3 and confirms v8 is a different major", () => {
    ...
    expect(v8Pkg.version.startsWith("8.")).toBe(true); // the default lane still resolves v8
    expect(v8Pkg.version).not.toBe(aliasPkg.version);
  });
});
...
describe.skipIf(!hasPresets)("PixiParticleRenderer (v7) — user textures (P0.1)", () => {
  it("does not throw for a user: ref and renders a synchronous placeholder", async () => {
    ...
    expect(viewOf(r).texture.width).toBe(64);
    expect(viewOf(r).texture.destroyed).toBe(false);
    r.destroy();
  });

  it("swaps in the decoded texture and recomputes invFrameWidth once ready", async () => {
    const doc = loadDoc("rain");
    doc.layers[0]!.texture.ref = "user:test";
    ...
    expect(viewOf(r).texture.width).toBe(128);

    // invFrameWidth is now 1/128: a live particle's scale reflects the real width.
    for (let i = 0; i < 10; i++) fx.step(1 / 60);
    r.sync();
    const part = pcOf(r).children[0] as Sprite;
    // scale = size * invFrameWidth; with size ~10..16 and invFrameWidth 1/128, scale < 0.2.
    expect(part.scale.x).toBeGreaterThan(0);
    expect(part.scale.x).toBeLessThan(16 / 128 + 1e-6);
    r.destroy();
  });
  ...
  it("keeps the placeholder and warns if decode fails", async () => {
    ...
    await r.ready;
    expect(viewOf(r).texture.width).toBe(64); // still the placeholder
    expect(r.warnings.some((w) => w.includes("broken"))).toBe(true);
    r.destroy();
  });
});

describe.skipIf(!hasPresets)("PixiParticleRenderer (v7) — default loader never fetches", () => {
  it("falls back to the placeholder on a remote-URL texture without calling fetch", async () => {
    ...
    try {
      ...
      expect(viewOf(r).texture.width).toBe(64); // soft-circle placeholder kept
...
describe.skipIf(!hasPresets)("PixiParticleRenderer (v7) — render list tracks live count, not capacity (P4.2)", () => {
  it("keeps children equal to the live count as it rises and falls", () => {
    ...
    const r = new PixiParticleRenderer(fx);
    const pc = pcOf(r);
    // Nothing simulated yet: the container holds no children despite the pool
    // being preallocated to maxParticles.
    expect(pc.children.length).toBe(0);
    ...
    expect(pc.children.length).toBe(live);
    expect(pc.children.every((p) => (p as Sprite).alpha > 0)).toBe(true);
    ...
    expect(pc.children.length).toBe(0); // drained, nothing to upload or draw
    r.destroy();
  });
  ...
  it("caps the render list far below capacity for a sparse high-capacity layer", () => {
    ...
    expect(pcOf(r).children.length).toBe(live); // not 5000
    r.destroy();
  });
  ...
  it("re-renders a slot correctly after it dies and is reused (burst → die → burst)", () => {
    ...
    r.sync();
    expect(pc.children.length).toBe(fx.layers[0]!.count);
    ...
    const v = viewOf(r);
    const reused = pc.children[0] as Sprite;
    expect(reused.scale.x).toBeCloseTo(v.buffers.size[0]! * v.invFrameWidth, 10);
    expect(reused.scale.x).toBeLessThan(1); // not a full-texture giant
    r.destroy();
  });
});

describe.skipIf(!hasPresets)("PixiParticleRenderer (v7) — every per-frame attribute is GPU-dynamic", () => {
  // Regression (v7 analog of the v8 "giant particle" guard): the scale-carrying
  // dynamic flag is `vertices` (aliased `scale`), NOT v8's `vertex`; alpha rides
  // the `tint` attribute. Left static, `vertices` uploads once at first render,
  // freezing every not-yet-live pool slot at scale 1 → a slot that goes live
  // later renders as a full-texture "giant particle" and size never animates.
  // Goldens can't catch this (each golden render IS a first render), so guard
  // the container's `_properties` ([vertices, position, rotation, uvs, tint])
  // directly.
  it("marks vertices, position, rotation, and tint dynamic on each ParticleContainer", () => {
    const doc = loadDoc("rain");
    const r = new PixiParticleRenderer(new Effect(doc, { seed: doc.seed }));
    for (const child of r.container.children) {
      const props = propsOf(child as ParticleContainer);
      expect(props[0], "vertices/scale must be dynamic").toBe(true);
      expect(props[1], "position must be dynamic").toBe(true);
      expect(props[2], "rotation must be dynamic").toBe(true);
      expect(props[4], "tint/alpha must be dynamic").toBe(true);
    }
    r.destroy();
  });
});
...
describe.skipIf(!hasPresets)("PixiParticleRenderer (v7) — flipbook rendering (P4.1)", () => {
  it("slices a cols×rows sheet row-major with a top-left origin", () => {
    ...
    const rect = (i: number) => ({
      x: v.frames![i]!.frame.x,
      y: v.frames![i]!.frame.y,
      w: v.frames![i]!.frame.width,
      h: v.frames![i]!.frame.height,
    });
  ...
  it("scales by frame width, not sheet width", () => {
    ...
    // scale.x == renderSize * invFrameWidth (frame basis), not / sheetWidth.
    expect(part.scale.x).toBeCloseTo(v.buffers.size[0]! * v.invFrameWidth, 10);
    expect(part.scale.x).not.toBeCloseTo(v.buffers.size[0]! / 64, 6); // would be the sheet-width bug
    r.destroy();
  });
...
describe.skipIf(!hasPresets)("PixiParticleRenderer (v7) — velocity alignment + speed stretch (M1)", () => {
  it("sets rotation from velocity and stretches scale.x (≠ scale.y) when render is set", () => {
    const doc = loadDoc("rain");
    ...
    for (let j = 0; j < count; j++) {
      ...
      const part = pc.children[j] as Sprite;
      // rotation follows the velocity vector (buffer holds degrees, renderer
      // converts to radians), so part.rotation === atan2 (radians) directly.
      if (speed >= 1e-3) expect(part.rotation).toBeCloseTo(Math.atan2(vy, vx), 5);
      // scale.x carries the stretch; scale.y does not → they differ for any moving particle.
      const stretch = Math.min(6, Math.max(1, 1 + 0.02 * speed));
      expect(part.scale.x).toBeCloseTo(part.scale.y * stretch, 6);
      if (stretch > 1.0001) {
        expect(part.scale.x).not.toBeCloseTo(part.scale.y, 4);
        sawStretch = true;
      }
    }
  ...
  it("randomFlip X (prob 1) mirrors scale.x only: scale.x = −scale.y, rotation === pool.rotation (M5)", () => {
    const doc = loadDoc("rain");
    const l = doc.layers[0]!;
    ...
    for (let j = 0; j < count; j++) {
      ...
      expect(part.scale.x).toBeLessThan(0); // X mirrored
      expect(part.scale.y).toBeGreaterThan(0); // Y untouched
      expect(part.scale.x).toBeCloseTo(-part.scale.y, 6); // magnitude preserved
    }
    r.destroy();
  });

  it("randomFlip Y (prob 1) mirrors scale.y only (negative scale, not UV flip) (M5)", () => {
    const doc = loadDoc("rain");
    const l = doc.layers[0]!;
    ...
    for (let j = 0; j < count; j++) {
      const part = pc.children[j] as Sprite;
      expect(part.scale.y).toBeLessThan(0); // Y mirrored
      expect(part.scale.x).toBeGreaterThan(0); // X untouched
      expect(part.scale.x).toBeCloseTo(-part.scale.y, 6);
    }
    r.destroy();
  });

  it("randomFlip X+Y with render stretch: flip applied AFTER stretch (|scale.x| keeps the stretch)", () => {
    const doc = loadDoc("rain");
    const l = doc.layers[0]!;
    ...
    for (let j = 0; j < count; j++) {
      const part = pc.children[j] as Sprite;
      expect(part.scale.x).toBeLessThan(0);
      expect(part.scale.y).toBeLessThan(0);
      const speed = Math.hypot(p.velX[j]!, p.velY[j]!);
      const stretch = Math.min(6, Math.max(1, 1 + 0.02 * speed));
      // |scale.x| = |scale.y|·stretch — the flip negates but does not erase the stretch.
      expect(Math.abs(part.scale.x)).toBeCloseTo(Math.abs(part.scale.y) * stretch, 5);
  ...
  it("render:null keeps scale.x === scale.y and rotation from pool.rotation (existing body)", () => {
    ...
    for (let j = 0; j < count; j++) {
      const part = pc.children[j] as Sprite;
      expect(part.scale.x).toBe(part.scale.y);
...
describe.skipIf(!hasPresets)("PixiParticleRenderer (v7) — built-in texture cache self-heal (P0.2)", () => {
...
describe.skipIf(!hasPresets)("PixiParticleRenderer (v7) — emitter placement (schemaVersion 2)", () => {
  ...
  it("a world layer's particles advance in world coords while its container stays put", () => {
    ...
    const part = pc.children[0] as Sprite;
