...
describe("validateParticle — document rules", () => {
  ...
  it("rejects schemaVersion < 1 or non-integer", () => {
    expect(firstCode(makeDoc({ schemaVersion: 0 as 4 }))).toBe("invalid-version");
    expect(firstCode(makeDoc({ schemaVersion: 1.5 as 4 }))).toBe("invalid-version");
  });

  it("refuses a newer schemaVersion (E11)", () => {
    expect(firstCode(makeDoc({ schemaVersion: 5 as 4 }))).toBe("newer-version");
  });
